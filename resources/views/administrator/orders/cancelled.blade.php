...
<div class="container-fluid my-4">
    <p class="title-md text-uppercase">Cancelled Order ( {{$total}} )</p>
</div>

<div class="container-fluid my-4">
    <form method="get" action="/administrator/orders/cancelled">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 mb-3">
                ...
                @if($order->wallet)
                    ...
                    @if($order->wallet()->count() > 0)

                        @if($order->wallet()->where('narration', 'Refund')->count() > $order->wallet()->where('narration', 'Refund Returned')->count())

                            <a onClick="return confirm('Do you really want to return refund for this order?')" href="/administrator/orders/{{$order->id}}/refund_return" class="btn btn-sm btn-outline-primary">Refund Return</a>

                        @else

                            <a onClick="return confirm('Do you really want to refund for this order?')" href="/administrator/orders/{{$order->id}}/refund" class="btn btn-sm btn-outline-primary">Refund Amount</a>
